...
def process_data(df_dash, df_act, df_deals):
                 ...
                 if exec_adv > 0:
                     ...
                     if not deal_txns.empty:
                         ...
                         if not recouped_rows.empty:
                             # Store both date and cumulative amount at recoupment
                             recoupment_map[did] = {
                                 'date': recouped_rows.iloc[0]['Period End Date'],
                                 'cum': recouped_rows.iloc[0]['Cum']
                             }
    ...
    for _, row in df_dash.iterrows():
        ...
        recoup_info = recoupment_map.get(did)
        
        if recoup_info:
            recoup_date = recoup_info['date']
            cum_at_recoup = recoup_info['cum']
        ...
        g, r, e, el_m, exp_prog, is_leg, proj_rec = calculate_pace_metrics(row, count, deal_calc_date, recent_velocity=recent_vel, cum_receipts_override=cum_at_recoup)
...
def show_portfolio(df_dash, df_act, current_date_override):
    ...
    with col3:
        st.empty() # Changed to empty just in case st.write("") is doing something weird, though unlikely.
...
def show_detail(df_dash, df_act, deal_id):
    ...
    <span class="diagnostic-label">PACE SCORE (AS-OF RECOUP):</span> <span class="diagnostic-value">{pace_ratio:.2f}x</span><br>
    ...
    <span class="diagnostic-label">PACE RATIO:</span> <span class="diagnostic-value">{pace_ratio:.2f}x</span>{artist_type_line}{legacy_flag}
        ...
        if col_early_reup and t_tier in ["GREENLIGHT", "WATCHLIST"]:
            with col_early_reup:
                ...
                # --- NEW: Suggested Re-Up Range ---
                if t_run > 0:
                    high_reup = floor_to(t_run * 12, 500)
                    low_reup = floor_to(high_reup * 0.7, 500)
                    reup_range_str = f"${low_reup:,.0f} – ${high_reup:,.0f}"
                else:
                    reup_range_str = "N/A"
                ...
                <span class="diagnostic-label">SUGGESTED RE-UP RANGE:</span> <span class="diagnostic-value" style="color: #ffd700;">{reup_range_str}</span><br>
    ...
    if not deal_act.empty:
        ...
        if not deal_act.empty:
            ...
            if is_no_adv:
                ...
                # CHANGE 2: Add Run Rate
                rr_rec = deal_row.get('Recent Velocity', 0)
                rr_suffix = ""
                if rr_rec > 0:
                    rr_suffix = f" | RUN RATE: ~${rr_rec:,.0f}/mo"
