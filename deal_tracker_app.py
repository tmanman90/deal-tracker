...
def show_portfolio(df_dash, df_act, current_date_override):
    ...
    if not df_act.empty and not df_dash.empty:
        ...
        if pulse_data:
            ...
            lines = alt.Chart(pulse_df).mark_line(interpolate='linear', strokeWidth=2).encode(
                x=alt.X('MonthIndex', title='Months Since Launch', axis=alt.Axis(domain=False, tickSize=0, grid=True, gridColor='#333333', gridDash=[4, 4], labelColor='#33ff00', titleColor='#ffbf00')),
                y=alt.Y('PctRecouped', title='Recoupment %', axis=alt.Axis(format='%', domain=False, tickSize=0, grid=True, gridColor='#333333', gridDash=[4, 4], labelColor='#33ff00', titleColor='#ffbf00')),
            ...
            area = alt.Chart(pulse_df).mark_area(interpolate='linear', opacity=0.1, color=alt.Gradient(gradient='linear', stops=[alt.GradientStop(color='#33ff00', offset=0), alt.GradientStop(color='rgba(0, 0, 0, 0)', offset=1)], x1=1, x2=1, y1=1, y2=0)).encode(x='MonthIndex', y='PctRecouped', color=alt.Color('Artist', scale=alt.Scale(range=neon_range), legend=None))
            pulse_chart = (area + lines).properties(height=300, width='container', background='transparent').configure_view(strokeWidth=0, fill=None)
...
def show_detail(df_dash, df_act, deal_id):
    ...
    if deal_subset.empty:
        st.error(f"ERROR: Deal ID {deal_id} not found.")
        if st.button("RESET"):
    ...
    if st.button("<< RETURN TO DASHBOARD"):
        del st.session_state['selected_deal_id']
        st.rerun()
    artist_name = deal_row.get('Artist / Project', deal_row.get('Artist', deal_row.get('Project', 'UNKNOWN ARTIST')))
    ...
    with col_gauge:
        ...
        if pace_ratio < 0.60: bar_color = 'red' 
        elif pace_ratio < 0.90: bar_color = 'orange' 
        else: bar_color = '#33ff00' 
        gauge_df = pd.DataFrame({'val': [chart_val], 'label': ['Recoupment'], 'color': [bar_color], 'Recoupment': [f"{actual_recoup*100:.1f}%"], 'Forecast': [f"{expected_recoup*100:.1f}%"]})
        gauge = alt.Chart(gauge_df).mark_bar(size=40).encode(x=alt.X('val', scale=alt.Scale(domain=[0, 1.0]), title="Recoupment Progress (0% - 100%)", axis=alt.Axis(format='%')), color=alt.Color('color', scale=None, legend=None), tooltip=['label', 'Recoupment', 'Forecast']).properties(height=80, title="RECOUPMENT METER")
    ...
    if not deal_act.empty:
        ...
        if not deal_act.empty:
            ...
            for i in range(1, max_month + 1):
                forecast_data.append({'MonthIndex': i, 'ForecastCum': monthly_forecast * i, 'Type': 'Forecast'})
            df_forecast = pd.DataFrame(forecast_data)
            c1, c2 = st.columns(2)
            with c1:
                bar = alt.Chart(deal_act).mark_bar(color='#b026ff').encode(x=alt.X('MonthLabel', sort=None, title='Period'), y=alt.Y('Net Receipts', title='Net Receipts'), tooltip=['MonthLabel', 'DateStr', 'Net Receipts']).properties(title="MONTHLY ACTUALS")
                st.altair_chart(bar, use_container_width=True)
            with c2:
                line_actual = alt.Chart(deal_act).mark_line(color='#33ff00', point=True).encode(x=alt.X('MonthLabel', sort=None, title='Period'), y=alt.Y('CumNet', title='Cumulative Net'), tooltip=['MonthLabel', 'DateStr', 'CumNet'])
                if not df_forecast.empty:
                    df_forecast['MonthLabel'] = df_forecast['MonthIndex'].apply(lambda x: f"M{x}")
                    line_forecast = alt.Chart(df_forecast).mark_line(color='#ffbf00', strokeDash=[5, 5]).encode(x=alt.X('MonthLabel', sort=None), y=alt.Y('ForecastCum'), tooltip=[alt.Tooltip('ForecastCum', title='Forecast Cumulative')])
                ...
                st.markdown("""<div style="text-align: center; font-size: 0.8rem;"><span style="color: #33ff00;">● Actual</span> &nbsp;&nbsp; <span style="color: #ffbf00;">--- Forecast (12mo Pace)</span></div>""", unsafe_allow_html=True)
            ...
            if remaining <= 0:
                ...
                st.markdown(f"""BASED ON LAST 3 MONTHS AVG (${last_rolling:,.0f}/mo):<br>ESTIMATED TOTAL TIME TO RECOUP: **{total_months:.1f} MONTHS**""", unsafe_allow_html=True)
            else:
